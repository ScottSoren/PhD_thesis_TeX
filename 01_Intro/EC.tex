...
Hydrogen (\ch{H2}) can be produced electrochemically by electrolysis of water into oxygen (\ch{O2}) and \ch{H2}. The overall reaction is:
\begin{equation}
...
\end{align}
They are connected by \textit{electrons} (\ch{e-} ) flowing through the wires and the power source, and by \textit{protons} (\ch{H+}) or other charge carriers moving through the liquid, which is therefore called an \textit{electrolyte}. The voltage, called the \textit{potential} difference, between the anode and cathode, is what drives the reaction. It makes the electron energy lower on the anode and higher on the cathode, pushing both reactions as written here in the forward direction. 
